...
def read_hyperparams(fname):
    ...
    max_steps        : maxEpisodeSteps (int, max number of steps per
                        episode)
    batch_size       : batchSize (int, size of batches used for
                        training)
    epsilon_start    : epsilonStart (float, initial value of
                        explore-exploit parameter)
    epsilon_stop     : epsilonStop (float, min value of explore-exploit
                        parameter)
    eps_decay_rate   : epsDecayRate (float, rate at which
                        explore-exploit param decays)
    discount         : gamma (float, reward discount rate)
    memory_size      : memSize (int, max number of experiences to store
                        in memory buffer)
    train_flag       : trainFlag (bool, if True, train the network. If
                        False, load saved)
    render_flag      : renderFlag (bool, if True, render scene during
                        testing)
    crop_top         : cropTop (int, number of rows to chop off top of
                        frame)
    crop_bot         : cropBot (int, number of rows to chop off bottom
                        of frame)
    crop_left        : cropLeft (int, number of cols to chop off left of
                        frame)
    crop_right       : cropRight (int, number of cols to chop off right
                        of frame)
    ...
    pretrain_len     : preTrainLen (int, number of experiences to
                        initially fill mem with)
    save_path        ; saveFilePath (string, path of checkpoint and
                        train params file)
    save_period      : savePeriod (int, save model every savePeriod
                        episodes)
    restart_training : restartTraining (int, if 1 start from beginning.
                        If 0, cont)
    ckpt_file        : ckptFile (string, name of the checkpoint file to
                        use when saving)
    test_flag        : testFlag (bool, whether or not to test the agent)
    env_name         : envName (string, name of the gym environment
                        (game) to use)
    architecture     : architecture (string, the network architecture to
                        use)
    fixed_Q          : fixedQ (int, 0 if not using, 1 if using the
                        fixed-Q technique)
    fixed_Q_steps    : fixedQSteps (int, steps between weights copies w/
                        fixed-Q)
    double_dqn       : doubleDQN (int, 0 if not using, 1 if using double
                        DQN technique)
    per              : per (int, 1 if using prioritized experience
                        replay, 0 if not)
    per_a            : perA (float, the alpha parameter in eq. 1 of
                        Schaul16)
    per_b            : perB (float, the beta parameter in IS weights of
                        Schaul16)
    per_e            : perE (float, the epsilon parameter in prop.
                        prior. of Schaul16)
