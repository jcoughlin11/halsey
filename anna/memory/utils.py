...
Date:    8/27/19
Purpose: Contains the functions related to setting up the agent's
         memory object.
Notes:
"""
...
def init_memory(params):
    """
    Figures out which type of memory object is to be used and creates
    an instance of it.

    Parameters:
    -----------
        params : dict
            Dictionary of params from the parameter file
    ...
    # Episode memory (for RNNs)
    if params['architecture'] in rnnRegister:
        # Non-priority
        if not params['enablePer']:
            memory = anna.memory.episode.EpisodeMemory(
                params['memSize'],
                params['pretrainLen'],
                params['pretrainNEp'],
                params['traceLen']
        ...
        # Priority
            ...
            raise ValueError("Priority Episode Memory not yet implemented!")
    # Priority memory (non-RNN)
    elif params['enablePer']:
        memory = anna.memory.priority.PriorityMemory(
            params['memSize'],
            params['pretrainLen'],
            perParams,
        )
    # Standard memory
        ...
        memory = anna.memory.standard.Memory(
            params['memSize'],
            params['pretrainLen'],
        )
    return memory
