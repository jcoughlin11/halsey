...
class Agent:
    ...
    def __init__(self, hyperparams, env, sess):
        ...
        # Use None for batch size in shape b/c in predict action we give
        # it 1 state, but in self.learn(), we give it self.batchSize
        # states all at once
        self.input_shape = (
            None,
            self.shrinkRows,
        ...
        # If applicable, build the second network for use with the
        # fixed-Q technique. This also creates the second network for
    ...
    def train(self, restart=True):
        ...
        for episode in range(start_ep, self.nEpisodes):
            ...
            # Stack and process initial state. State is returned as a
            # tensor of shape (frame_rows, frame_cols, stack_size).
            # frame_stack is the same data, but in the form of a deque
            ...
            while step < self.maxEpSteps:
                ...
                next_state, frame_stack = nu.stack_frames(
                    frame_stack,
                    next_state,
                    False,
                    self.stackSize,
                ...
                if done:
                    ...
                    if os.path.isfile(os.path.join(os.getcwd(), "stop")):
                        early_abort = True
                        subprocess32.call(
                            ["rm", os.path.join(os.getcwd(), "stop")]
                        )
                        break
                    break
                # Set up for next step
                    ...
                    if os.path.isfile(os.path.join(os.getcwd(), "stop")):
                        ...
                        break
            # If we finish because of a time out (i.e, we reach the max
            # number of steps, then we want to get the total episode
            # reward
    ...
    def choose_action(self, state, decay_step):
        ...
        # Exploit
            ...
            # tf needs the batch size as part of the shape. See comment
            # on self.input_shape in the constructor
            state = state.reshape(
    ...
    def learn(self):
        """
        Samples from the memory buffer and uses it to update the
        network weights.

        Our network is our Q table. It tells us, for every state, what
        the max discounted future reward is for each action. Our policy
        is to select the action with the largest max discounted future
        reward at each state. The network is an  approximation to the
        full Q table, since the full thing is too large to be
        represented exactly.

        We start by not knowing anything about the max discounted future
        reward for any (state, action) pair. By playing, the environment
        (game) gives us rewards (either positive or negative) for each
        action. This allows us to slowly fill in the Q table (network).

        However, how do we train the network (fill in the Q table)? That
        is, when we take an action, we are given a reward, but is that
        the max reward for that state? Further, how do we know what max
        discounted future reward each (state, action) pair gives rise
        to?

        The answer is to compare how well we would do continuing to play
        from the state our current action brings us to with how well we
        would do by playing optimally from our current state (where the
        optimal action is not necessarily the action we chose). The idea
        is that, if we're playing optimally, then the two should match.
        The trouble is that we don't actually know the Q table ahead of
        time, so we have to use both estimates, along with the rewards
        received from the game, in order to learn.

        The estimate of the "how well can I do playing optimally from
        the state my current action brings me to?" is given by the
        Bellmann equation. These are the target Q values.

        If the action brings us to a terminal state, then the max
        discounted future reward we can achieve by playing optimally
        from the state our action brought us to is just the reward given
        by the terminal state (since there are no other states after
        it).

        If the current action for the current state does not result in a
        terminal state, then we need to make an estimate about what the
        max discounted future reward will be (Bellmann equation).

        We then ask, "How well can I do if I play optimally from the
        current state onwards (where the optimal action is not
        necessarily the chosen action)?" This is the value we get from
        our Q table (network) Q_prediction. These values are updated
        constantly as the agent gets reward information from the
        environment.

        This way, information about rewards lets us get better Q_target
        values, which improve our Q_preidctions, which, along with more
        information about rewards, allows us to improve Q_target, and so
        on, forming a feedback loop.
        ...
        # Double dqn
        if self.doubleDQN == 1:
            # Double dqn attempts to deal with the following issue: when
            # we choose the action that gives rise to the highest Q
            # value for the next state, how do we know that that's
            # actually the best action? Since we're learning from
            # experience, our estimated Q values depend on which actions
            # have been tried and which neighboring states have been
            # visited. As such, double dqn separates out the estimate of
            # the Q value and the determination of the best action to
            # take at the next state. We use our primary network to
            # choose an action for the next state and then pass that
        ...
        for i in range(self.batchSize):
            # Begin by getting an estimate of the max discounted future
            # reward we can achieve by taking the chosen action and then
            # playing optimally from the state that action brings us to.
            # This is called Q_target. If the action brings us to a
            # terminal state, then the max discounted future reward we
            # can achieve by playing optimally from the state our action
            # brought us to is just the reward given by the terminal
            # state (since there are no other states after it)
            if dones[i]:
                targetQ[i] = rewards[i]
            # Otherwise, use the Bellmann equation
            else:
                # If using double dqn, this is where we apply the Q
                # value for the action chosen by our target network (see
                # eq in double_dqn if block above)
                if self.doubleDQN == 1:
                    action = np.argmax(Q_next[i])
                    targetQ[i] = (
                        rewards[i]
                        + self.discountRate * Q_target_next[i][action]
                    )
                # Normal DQL method: uses action that gives max Q value
                # for next state
                else:
                    targetQ[i] = rewards[i] + self.discountRate * np.amax(
                        Q_next[i]
                    )
        # Update network weights using the state as input and the
        # updated Q values as "labels." tf evaluates each element in the
        # list it's given and returns one value for each element. Note
        # that this is called outside of the above for loop in order to
        # minimize the number of calls to sess.run that are needed.
        # This requires that actions be reshaped to a column vector
        actions = actions.reshape((self.batchSize, 1))
        if self.per == 1:
            fd = {
                self.qNet.inputs: states,
                self.qNet.target_Q: targetQ,
                self.qNet.actions: actions,
                self.qNet.isWeights: isWeights,
            }
            _, loss, absErrors = self.sess.run(
                [self.qNet.optimizer, self.qNet.loss, self.qNet.absErrors],
                feed_dict=fd,
            )
            ...
            loss, _ = self.sess.run(
                [self.qNet.loss, self.qNet.optimizer], feed_dict=fd
            )
        return loss
    ...
    def test(self, render_flag):
        """
        Has the agent play the game using what it's learned so we can
        see how well it does.

        Parameters:
        -----------
            render_flag : int
                If 1, then we have gym draw the game to the screen for
                us so we can see what the agent is doing. Otherwise,
                nothing is drawn and only the final info about the
                agent's performance is given.
        ...
        for episode in range(1):
            ...
            while True:
                # tf needs the batch size as part of the shape. See
                # comment on self.input_shape in the constructor
                state = state.reshape(
                    (1, state.shape[0], state.shape[1], state.shape[2])
                )
                # Get what network thinks is the best action for current
                # state
                ...
                if done:
                    ...
                    break
                # If not done, get the next state and add it to the
                # stack of frames
                else:
                    next_state, frame_stack = nu.stack_frames(
                        frame_stack,
    ...
    def update_target_graph(self):
        """
        Handles copying over the parameters from qNet to targetQNet in
        ...
        --------
            opHolder : list
                This is a list of tensorflow variables from qNet to be
                copied over to targetQNet.
        """
        ...
        toVars = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, "targetQNet"
        )
        # Initialize the list to hold variables to be updated and the
        # values to update them with
        opHolder = []
        # Fill in the list
